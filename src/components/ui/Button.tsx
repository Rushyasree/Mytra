...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className = "", variant = "default", size = "default", loading = false, children, ...props }, ref) => {
    ...
    switch (variant) {
      ...
      case "outline": variantStyles = "border border-gray-300 bg-background hover:bg-gray-100 hover:text-gray-900"; break;
    ...
    return (
      <button
        className={`${baseStyles} ${variantStyles} ${sizeStyles} ${className} relative ${loading ? "text-transparent select-none pointer-events-none" : ""}`}
        ref={ref}
